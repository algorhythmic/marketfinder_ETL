...
interface FilterableTData {
  _id: string; // Assuming all data will have an _id for identification
...
interface DataTableProps<TData extends FilterableTData, TValue> {
  ...
  onSelectionChange?: (selectedRows: TData[]) => void; // New callback prop
}
...
export function MarketDataTable<TData extends FilterableTData, TValue>({
  ...
  React.useEffect(() => {
    if (table && onSelectionChange) { // Add guard for table instance
      const selectedRowsData = table.getSelectedRowModel().rows.map(row => row.original);
      onSelectionChange(selectedRowsData);
    }
    // table.getSelectedRowModel() might change reference, so include it if it causes issues.
    // For now, rowSelection is the primary dependency indicating a change in selection state.
