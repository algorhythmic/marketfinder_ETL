...
async function prepareCandidatePairs(connection: DuckDBConnection): Promise<MarketTuple[][]> {
  // Get high-value markets from both platforms
  const kalshiMarkets = await getTopMarkets(connection, 'kalshi', 100);
  const polymarketMarkets = await getTopMarkets(connection, 'polymarket', 100);
  ...
  const maxPairs = 50; // Start with reasonable batch size
...
async function getTopMarkets(connection: DuckDBConnection, platform: string, limit: number): Promise<MarketTuple[]> {
  const result = await connection.run(`
    SELECT id, platform, title, category, yes_price, volume, liquidity
  ...
  return rows.map(row => ({
    ...
    yes_price: Number(row[4]),
    volume: Number(row[5]),
    liquidity: Number(row[6])
  }));
}
...
Analyze these market pairs and determine if they represent the same underlying event/outcome. For each pair, provide a confidence score (0.0-1.0) and brief reasoning.
...
${batch.map((pair, i) => {
  ...
  return `PAIR ${i + 1}:
Kalshi: "${kalshi.title}" (${kalshi.category}, price: ${kalshi.yes_price.toFixed(3)})
Polymarket: "${polymarket.title}" (${polymarket.category}, price: ${polymarket.yes_price.toFixed(3)})`;
}).join('\n\n')}

RESPOND IN THIS EXACT FORMAT:
PAIR 1: confidence=0.XX, reasoning="brief explanation"
PAIR 2: confidence=0.XX, reasoning="brief explanation"
...
